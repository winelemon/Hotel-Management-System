...
public class UserController {
    ...
    @RequestMapping(method = RequestMethod.POST,value = "/selectByUsernameAndPassword")
    public AjaxResult selectByUsernameAndPassword(String username, String password){
        User user = userService.selectByUsernameAndPassword(username, password);
        if (user == null){
            return ResponseTool.failed(1002,"密码不对");
        }else {
            return ResponseTool.success("密码正确");
        }
    }
