...
<script>
  ...
  import { getUserInfo,userLogin,selectByUsernameAndPassword } from "@/api/user";
  ...
  export default {
      ...
      methods:{
        ...
        pay(){
            ...
            selectByUsernameAndPassword(username,this.password).then(res=>{
              if(res.code === 1000){
                this.$toast.success("密码正确进入支付")
                // window.open(`http://118.25.99.15:8080/alipay/pay?subject=${this.order.roomType}&traceNo=${this.order.orderId}&totalAmount=${this.order.orderCost}`)
                window.location.href = `http://121.41.226.28:8080/alipay/pay?subject=${this.order.roomType}&traceNo=${this.order.orderId}&totalAmount=${this.order.orderCost}`;

              }else{
                this.payLoading = false
                this.$toast.error("支付失败,请确认密码后重试")
              }
            })
